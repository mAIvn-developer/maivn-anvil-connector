...
def test_handler_returns_submitted_response() -> None:
    sessions.reset()
    sessions.bind_owner("s1", "secret-A")
    tables.put_interrupt("s1", interrupt_id="i1", prompt="Name?", input_type="text", choices=None)
    interrupts.submit_interrupt(
        session_id="s1", session_secret="secret-A", interrupt_id="i1", response="Ada"
    )
...
def test_private_response_is_deleted_after_consume() -> None:
    ...
    interrupts.submit_interrupt(
        session_id="s1", session_secret="secret-A", interrupt_id="i2", response="123-45-6789"
    )
...
def test_submit_rejects_wrong_secret() -> None:
    sessions.reset()
    sessions.bind_owner("s9", "secret-A")
    tables.put_interrupt("s9", interrupt_id="i9", prompt="?", input_type="text", choices=None)

    with pytest.raises(NotAuthorizedError):
        interrupts.submit_interrupt(
            session_id="s9", session_secret="wrong-secret", interrupt_id="i9", response="x"
        )
